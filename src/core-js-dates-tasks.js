...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return Math.ceil(Math.abs(endUTC - startUTC) / (1000 * 60 * 60 * 24)) + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  return check >= new Date(period.start) && check <= new Date(period.end);
}
...
function getWeekNumberByDate(date) {
  ...
  return Math.ceil((diff / oneDay + 1) / 7);
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDate <= endDate) {
    if (workDaysCount < countWorkDays) {
      ...
      if (workDaysCount === countWorkDays + countOffDays) {
        workDaysCount = 0;
      }
    }
